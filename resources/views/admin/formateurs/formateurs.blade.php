<x-main>
    ...
    <x-slot name="bodyContent">
        <div class="card card-custom">
            <div class="card-header">
                <div class="card-title">
                    ...
                    <h3 class="card-label">Listing des Formateurs</h3>
                </div>
        ...
        {{--        add formateur modal--}}
        <x-modal modal-title="Ajouter un formateur" method="POST" route="formateur.store" form-id="addForm"
                 modal-name="addModal">
...
{{--                <x-input-group labelclass="" id="evaluer" name="evaluer" labelname="Evaluer" class="form-control" type="text"--}}
{{--                               placeholder="Entrer un Evaluer" grid="" step="" min=""/>--}}
        ...
        <x-modal modal-title="Modifier un formateur" method="POST" route="formateurs.update" form-id="updateForm"
            ...
            <x-slot name="modalContent">
                ...
                <x-input-group labelclass="" id="telUpdate" name="tel" labelname="Tel" class="form-control" type="text"
                               placeholder="Entrer un Tel" grid="" step="" min=""/>
                <x-input-group labelclass="" id="evaluerUpdate" name="evaluer" labelname="Evaluer" class="form-control" type="text"
                               placeholder="Entrer un Evaluer" grid="" step="" min=""/>
    ...
    <x-slot name="jsSlot">
        ...
        <script>
            ...
            $(document).on('shown.bs.modal', '#roleModalUFormateur', function (evt) {
                select.trigger("change");
                $('.select2-ajax').trigger("change");
            });
        </script>
        <x-js-data-table route='{{route("formateurs.index")}}' tableId="kt_datatable">

            <x-slot name="jsonColumns">
                {!! file_get_contents(public_path("js/pages/formateur/formateurs_datatable.js")) !!}
            </x-slot>
        </x-js-data-table>
        <script>
            var id;
            ...
            $('body').on('click', '.edit', function () {
                id = $(this).data("id");
                $("#updateForm").attr("action", "{{route("formateurs.update",["formateur"=>""])}}" + "/" + id);
                ...
                $('#evaluerUpdate').val($(this).data("evaluer"));
                ...
                swal.fire({
                    ...
                    if (willDelete) {
                        $.ajax({
                            ...
                            url: "{{route('formateurs.desactivateAccount',["formateur" => ''])}}" + "/" + id,
